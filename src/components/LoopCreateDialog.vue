    ...
    <dialog ref="dialog" class="loop-create" aria-labelledby="loop-create-title" @cancel.prevent="close" @keydown.escape.stop>
      <form @submit.prevent="create">
        ...
        <div class="body">
          ...
          <details><summary>고급 설정</summary><div class="options"><label>기본 Usage 조회 간격<select v-model.number="polling"><option :value="30">30초</option><option :value="60">60초</option><option :value="120">120초 (권장)</option><option :value="300">300초</option></select><small>사용량이 높아지면 자동으로 더 자주 확인합니다</small></label><label>Interrupt timeout (초)<input v-model.number="interrupt" type="number" min="1" max="10" /></label></div><label class="participant"><input v-model="autoResume" type="checkbox" />Usage reset 후 자동 재개</label></details>
