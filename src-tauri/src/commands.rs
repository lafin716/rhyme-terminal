...
/// Directory segment allowlist for [`resolve_account_dir`]: only these two CLI
/// agents get an isolated per-profile login directory today (see the Accounts
/// settings tab). Keeping this a closed set (rather than accepting any string)
/// also doubles as the path-traversal guard for the `agent` segment.
const ACCOUNT_AGENTS: &[&str] = &["claude", "codex"];

fn validate_account_agent(agent: &str) -> Result<(), String> {
    if ACCOUNT_AGENTS.contains(&agent) {
        Ok(())
    } else {
        Err(format!("Unsupported account agent: {agent}"))
    }
}

/// A profile id is generated by the frontend (`useAccountProfiles.ts`) and is
/// never user-typed, but this still guards the filesystem join against a
/// malformed or malicious value (e.g. `..` or a path separator) before it
/// becomes a directory name.
fn validate_account_profile_id(profile_id: &str) -> Result<(), String> {
    ...
    if valid {
        ...
        Err("Invalid account profile id".to_string())
    }
}
...
/// Resolves (creating if necessary) the isolated login/config directory for one
/// account profile, so it can be handed to a spawned session as
/// `CLAUDE_CONFIG_DIR`/`CODEX_HOME` (see `useAccountProfiles.ts`). Each
/// `(agent, profile_id)` pair always maps to the same directory under the
/// app's local-data folder, so re-adding a profile with the same id reuses any
/// login it already holds.
#[tauri::command]
pub fn resolve_account_dir(
    app: AppHandle,
    agent: String,
    profile_id: String,
    ...
    let base = app
        .path()
        .app_local_data_dir()
        .map_err(|e| format!("Unable to resolve the app data directory: {e}"))?;
    let dir = account_dir_path(&base, &agent, &profile_id)?;
    std::fs::create_dir_all(&dir)
        .map_err(|e| format!("Failed to create {}: {e}", dir.display()))?;
    Ok(dir.to_string_lossy().into_owned())
}

/// File name a `setup-token`-linked profile's token is stored under, inside
/// its own isolated account directory (see `account_dir_path`). Kept out of
/// the frontend's `AccountProfile`/localStorage entirely — see
...
/// The profile's directory must already exist (via `resolve_account_dir`).
#[tauri::command]
...
/// Reads back a token saved by [`set_account_token`], or `None` if this
/// profile has never linked one (e.g. it uses the default OAuth login).
#[tauri::command]
...
mod account_dir_tests {
    ...
    #[test]
    fn rejects_an_unsupported_agent() {
        assert!(account_dir_path(Path::new(r"C:\data"), "bash", "acct_1").is_err());
    }

    #[test]
    fn rejects_an_empty_profile_id() {
        assert!(account_dir_path(Path::new(r"C:\data"), "codex", "").is_err());
    }

    #[test]
    fn rejects_a_path_traversal_profile_id() {
        assert!(account_dir_path(Path::new(r"C:\data"), "codex", "..").is_err());
        assert!(account_dir_path(Path::new(r"C:\data"), "codex", "../evil").is_err());
        assert!(account_dir_path(Path::new(r"C:\data"), "codex", "a/b").is_err());
        assert!(account_dir_path(Path::new(r"C:\data"), "codex", r"a\b").is_err());
    }
