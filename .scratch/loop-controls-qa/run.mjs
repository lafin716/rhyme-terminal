...
const dir=path.resolve('.scratch/loop-controls-qa'),profile=path.join(dir,`chrome-${Date.now()}`);
...
try {
 ...
 await evaluate("document.querySelector('.tab.active').dispatchEvent(new MouseEvent('dblclick',{bubbles:true}))");await delay(300);
 await evaluate("const i=document.querySelector('.tab.active input');if(!i)throw Error('Rename input missing');i.value='QA renamed loop';i.dispatchEvent(new Event('input',{bubbles:true}));i.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}))");await delay(700);
 if(!await evaluate("document.querySelector('.tab.active .name').textContent.includes('QA renamed loop')"))throw Error('Tab rename failed');
 if(!await evaluate("window.__groups[0].id==='g1' && window.__groups[0].activeSessionId==='p1'"))throw Error('Rename changed identity');
 await shot('renamed');
 const results={errors,loopCalls:await evaluate("window.__qaCalls.filter(x=>x.command==='loop_request').map(x=>x.args.request.op)")};
 if(!results.loopCalls.includes('rename'))throw Error('Rename IPC missing');
