...
const dir=path.resolve('.scratch/loop-create-qa'),profile=path.join(dir,`chrome-${Date.now()}`);
...
try {
 ...
 if(r.op==='capabilities')return {version:1,explicitStart:true};
 if(r.op==='configure')return r.settings;
 if(r.op==='list')return JSON.parse(JSON.stringify(window.__groups));
 if(r.op==='conversations'){if(window.__listError)throw Error('목록 조회 실패');return window.__emptyList?[]:[{sessionId:'older-session',candidateKey:'claude:qa-claude',agent:'claude',label:'Work Claude',title:'장바구니 결제 오류 수정',updatedAt:1788940000000},{sessionId:'newer-session',candidateKey:'codex:qa-codex',agent:'codex',label:'Work Codex',title:'상품 검색 화면 구현',updatedAt:1788941000000}];}
 if(r.op==='create'){if(window.__failCreate)throw Error('선택한 대화가 이동되었습니다. 다시 선택하세요.');const g={id:'g1',name:r.name,workspaceId:r.workspaceId,cwd:r.cwd,status:'running',activeSessionId:'p1',reason:null,updatedAt:1,attempts:[{id:'a1',sessionId:'p1',agent:'claude',profileId:'qa-claude',label:'Work Claude',status:'running',reason:null,startedAt:1,endedAt:null}]};window.__groups.push(g);return g;}
 if(r.op==='history')return btoa('Completed attempt output - read only');
 const g=window.__groups.find(g=>g.id===r.id);
 if(r.op==='rename')g.name=r.name;
 ...
 const count=()=>evaluate("window.__qaCalls.filter(x=>x.command==='loop_request'&&x.args.request.op==='create').length");
 await assert("!!document.querySelector('dialog[open]')",'Creation dialog absent');
 if(await count()!==0)throw Error('Opening dialog started work');
 await assert("document.querySelector('.loop-create .primary').disabled",'Empty prompt enabled');
 await evaluate("var p=document.querySelector('#loop-prompt');p.value='새 결제 모듈 구현';p.dispatchEvent(new Event('input',{bubbles:true}))");await delay(100);
 await shot('new-prompt');
 await click('.mode input[value=session]');await delay(300);
 await assert("document.querySelectorAll('.loop-create .session input:checked').length===0 && document.querySelector('.loop-create .primary').disabled",'Session auto-selected');
 await shot('session-picker');
 await evaluate("var p=document.querySelector('.list-tools input');p.value='결제';p.dispatchEvent(new Event('input',{bubbles:true}))");await delay(100);
 await assert("document.querySelectorAll('.loop-create .session').length===1",'Search failed');
 await click('.loop-create .session input');
 await assert("!document.querySelector('.loop-create .primary').disabled",'Explicit choice not enabled');
 if(await count()!==0)throw Error('Selection started work');
 await evaluate('window.__failCreate=true');await click('.loop-create .primary');
 await assert("!!document.querySelector('.loop-create .error') && !!document.querySelector('dialog[open]')",'Create error lost form');
 await evaluate('window.__failCreate=false');await click('.loop-create .primary');
 await assert("!document.querySelector('dialog[open]') && window.__qaCalls.filter(x=>x.command==='loop_request'&&x.args.request.op==='create').at(-1).args.request.start.sessionId==='older-session'",'Wrong conversation resumed');
 await click('.terminal-menu-toggle');await evaluate("[...document.querySelectorAll('.terminal-option')].find(x=>x.textContent.includes('새 에이전트 루프')).click()");await delay(300);
 await evaluate("var p=document.querySelector('#loop-prompt');p.value='신규 작업만 시작';p.dispatchEvent(new Event('input',{bubbles:true}))");
 await click('.mode input[value=session]');await delay(300);await click('.mode input[value=prompt]');
 await assert("document.querySelector('#loop-prompt').value==='신규 작업만 시작'",'Draft lost on mode change');
 await cdp('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:false});await delay(300);await shot('narrow-prompt');
 ...
 await click('.loop-create .primary');
 await assert("window.__qaCalls.filter(x=>x.command==='loop_request'&&x.args.request.op==='create').at(-1).args.request.start.prompt==='신규 작업만 시작'",'Prompt not delivered');
 for(let i=0;i<50;i++){if(!await evaluate("!!document.querySelector('dialog[open]')"))break;await delay(100);}
 await cdp('Emulation.setDeviceMetricsOverride',{width:1440,height:1000,deviceScaleFactor:1,mobile:false});await delay(600);
 await click('.terminal-menu-toggle');await evaluate("[...document.querySelectorAll('.terminal-option')].find(x=>x.textContent.includes('새 에이전트 루프')).click()");await delay(300);
 await evaluate('window.__emptyList=true');await click('.mode input[value=session]');await delay(300);await shot('empty-list');
 await assert("document.querySelector('.loop-create .primary').disabled && document.querySelector('.empty').textContent.includes('이어갈 대화가 없습니다')",'Empty list state incorrect');
 const beforeCancel=await count();await click('.loop-create .icon-button');if(await count()!==beforeCancel)throw Error('Cancel started work');
 const results={errors,checks:['open-without-execution','prompt-required','explicit-session-choice','search','error-retains-form','exact-selected-session','draft-preserved','narrow-viewport','exact-prompt','empty-list','cancel-without-execution']};
 await fs.writeFile(path.join(dir,'report.json'),JSON.stringify(results,null,2));console.log(JSON.stringify(results,null,2));if(errors.length)throw Error('Runtime errors');
