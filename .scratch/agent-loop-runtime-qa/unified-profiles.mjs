...
try {
 ...
 await assert("document.querySelector('.profile-zone:first-child .candidate .provider-tag').textContent==='Codex' && document.querySelector('.profile-zone:first-child .candidate .order').textContent==='1'",'Codex did not move physically to the first row');
 await shot('settings-profiles-unified');
 ...
 await evaluate("Object.assign(window.__groups[0],{status:'running',activeProfile:'codex:b',currentProvider:'codex',runtime:{status:'RUNNING',provider:'codex',pid:42}})");await evaluate("window.__groups[0].profiles[1].status='ACTIVE'");await delay(2500);await shot('running');
 ...
 await evaluate("Object.assign(window.__groups[0],{status:'waiting_for_usage_reset',waitingProfileId:'codex:a',resumeAt:Date.now()+120000,runtime:{status:'IDLE',provider:null,pid:null}})");await delay(2500);await shot('waiting');
 ...
 const result={errors,overflow,checks:['no-session-selector','unified-profile-lists','provider-tags','codex-first-row','drag-reorder','active-only','auto-start-terminal','collapsed-by-default','active-profile','usage-bars','countdown','desktop-and-narrow']};
