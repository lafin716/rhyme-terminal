...
try {
 ...
 await click('.loop-create input[type=checkbox]');await shot('create-desktop');
 ...
 await assert("document.querySelector('.loop-toolbar').textContent.includes('Agent 대기 중')",'Idle status missing');
 ...
 const result={errors,overflow,checks:['no-session-selector','create-does-not-start-agent','idle-terminal','collapsed-by-default','active-profile','usage-bars','countdown','desktop-and-narrow']};
