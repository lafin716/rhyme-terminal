...
try {
 ...
 if(r.op==='capabilities')return {version:1,runtimeMonitor:true,autoStart:true};if(r.op==='configure')return r.settings;
 if(r.op==='list')return JSON.parse(JSON.stringify(window.__groups));
 if(r.op==='create'){const g={id:'g1',name:r.name,workspaceId:r.workspaceId,cwd:r.cwd,status:'resuming',activeSessionId:'p1',reason:null,updatedAt:1,attempts:[],runtime:{status:'IDLE',provider:null,pid:null},activeProfile:null,events:[{at:Date.now(),kind:'CREATED',message:'Agent 대기 중'}],profiles:[{key:'codex:a',agent:'codex',label:'Work 01',status:'EXHAUSTED',usage:91,threshold:90,remaining:9,resetAt:Date.now()+3600000,error:null},{key:'codex:b',agent:'codex',label:'Work 02',status:'AVAILABLE',usage:43,threshold:90,remaining:57,resetAt:null,error:null}]};window.__groups.push(g);return g;}
 ...
 await assert("document.querySelectorAll('.profile-list').length===2 && !document.querySelector('.provider-columns') && document.querySelectorAll('.provider-tag').length===4",'Unified profile lists or tags missing');
 ...
 await evaluate("window.__dragEvents=[];['dragstart','dragend','drop','pointercancel'].forEach(t=>document.addEventListener(t,e=>window.__dragEvents.push(t),true));document.querySelector('.profile-board').scrollIntoView({block:'center'})");await delay(200);
 const points=await evaluate("(()=>{const rows=[...document.querySelectorAll('.profile-zone:first-child .candidate')];const a=rows.find(x=>x.textContent.includes('Work Codex')).querySelector('.drag-handle').getBoundingClientRect(),b=rows[0].getBoundingClientRect();return {x:a.x+a.width/2,y:a.y+a.height/2,tx:b.x+80,ty:b.y+5}})()");
 await cdp('Input.dispatchMouseEvent',{type:'mouseMoved',x:points.x,y:points.y});
 await cdp('Input.dispatchMouseEvent',{type:'mousePressed',x:points.x,y:points.y,button:'left',clickCount:1});
 for(let i=1;i<=15;i++){await cdp('Input.dispatchMouseEvent',{type:'mouseMoved',x:points.x+(points.tx-points.x)*i/15,y:points.y+(points.ty-points.y)*i/15,buttons:1});await delay(25);}
 await cdp('Input.dispatchMouseEvent',{type:'mouseReleased',x:points.tx,y:points.ty,button:'left',clickCount:1});await delay(300);
 const result=await evaluate("({first:document.querySelector('.profile-zone:first-child .candidate .provider-tag').textContent,events:window.__dragEvents})");console.log(JSON.stringify(result));await shot('actual-drag');
 if(result.first!=='Codex')throw Error('Real mouse drag failed');
 const mouseDrag=async(targetZone)=>{
  const p=await evaluate("(()=>{const handle=[...document.querySelectorAll('.candidate')].find(r=>r.textContent.includes('Work Codex')).querySelector('.drag-handle').getBoundingClientRect();const zone=document.querySelectorAll('.profile-zone')["+targetZone+"];const row=zone.querySelector('.candidate');const target=(row??zone).getBoundingClientRect();return {x:handle.x+handle.width/2,y:handle.y+handle.height/2,tx:target.x+100,ty:target.y+5}})()");
  await cdp('Input.dispatchMouseEvent',{type:'mouseMoved',x:p.x,y:p.y});await cdp('Input.dispatchMouseEvent',{type:'mousePressed',x:p.x,y:p.y,button:'left',clickCount:1});
  for(let i=1;i<=20;i++){await cdp('Input.dispatchMouseEvent',{type:'mouseMoved',x:p.x+(p.tx-p.x)*i/20,y:p.y+(p.ty-p.y)*i/20,buttons:1});await delay(20);}
  await cdp('Input.dispatchMouseEvent',{type:'mouseReleased',x:p.tx,y:p.ty,button:'left',clickCount:1});await delay(200);
 };
 await mouseDrag(1);
 await assert("document.querySelectorAll('.profile-zone:first-child .candidate').length===1",'Mouse drag to waiting failed');
 await mouseDrag(0);
 await assert("document.querySelector('.profile-zone:first-child .candidate .provider-tag').textContent==='Codex'",'Mouse drag back to active failed');
 await click('.loop-settings button[type=submit]');
 await assert("JSON.parse(localStorage.getItem('winmux:loop-routing:v1')).candidates.filter(c=>c.enabled)[0].agent==='codex'",'Mouse reordered settings not saved');
 console.log('Real mouse reorder, cross-zone movement and persistence passed');

 await click('.back-to-app');await click('.terminal-menu-toggle');
 await evaluate("[...document.querySelectorAll('.terminal-option')].find(x=>x.textContent.includes('새 에이전트 루프')).click()");await delay(400);await click('.loop-create .primary');await delay(1200);
 await evaluate("Object.assign(window.__groups[0],{status:'running',activeProfile:'claude:qa-claude',currentProvider:'claude',runtime:{status:'RUNNING',provider:'claude',pid:42},profiles:[{key:'claude:qa-claude',agent:'claude',label:'Setup token',status:'ACTIVE',usage:null,threshold:90,remaining:null,resetAt:null,error:null,usagePending:true}]})");await delay(2600);await click('.loop-toolbar .expand');
 await assert("!!document.querySelector('.usage-pending') && !document.querySelector('.profile-error')",'Deferred usage shown as an error');await shot('usage-pending');console.log('Deferred usage UI passed');
