...
const dir=path.resolve('.scratch/session-menu-drag-qa'),profile=path.join(dir,`chrome-${Date.now()}`);
...
try {
 ...
 const drag = async (source,target,after=false,commit=true)=> {
  await evaluate(`(()=>{const handle=document.querySelector('[data-menu-item="${source}"] .drag-handle'); const row=document.querySelector('[data-menu-item="${target}"]'); const dataTransfer=new DataTransfer(); const rect=row.getBoundingClientRect(); const options={bubbles:true,cancelable:true,dataTransfer,clientY:rect.top+(${after}?rect.height-2:2)};handle.dispatchEvent(new DragEvent('dragstart',options));row.dispatchEvent(new DragEvent('dragover',options));if(${commit})row.dispatchEvent(new DragEvent('drop',options));handle.dispatchEvent(new DragEvent('dragend',options));})()`);
  await delay(100);
 };
 const order=()=>evaluate("Array.from(document.querySelectorAll('[data-menu-item]')).map(e=>e.dataset.menuItem)");
 const initial=await order();
 await drag('page','default',true);
 if((await order())[3]!=='page')throw Error('Downward drag failed');
 await drag('page','claude');
 if(JSON.stringify(await order())!==JSON.stringify(initial))throw Error('Upward drag failed');
 await drag('page','default',true,false);
 await drag('page','page');
 if(JSON.stringify(await order())!==JSON.stringify(initial))throw Error('Cancel or self-drop changed order');
 await drag('codex','page');
 if(await evaluate("!!document.querySelector('.position')"))throw Error('Position numbers remain');
