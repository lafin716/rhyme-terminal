...
try {
 ...
 assert(await evaluate("document.querySelector('.target').textContent==='src/main.ts:12:3'"),'Wrong target');
 ...
 assert(await evaluate("__qaCalls.some(c=>c.command==='plugin:opener|reveal_item_in_dir')"),'System browser action missing');
 await click(10);
 assert(await evaluate("!document.querySelector('.link-menu')"),'Direct shortcut opened menu');
 assert(errors.length===0,JSON.stringify(errors));
 console.log('PASS: real xterm file path click, menu target, Escape, system browser, Ctrl+Shift+click; no browser exceptions');
