...
try {
 ...
 localStorage.setItem('winmux:accounts:v1',JSON.stringify({version:1,items:[{id:'qa-codex',agent:'codex',label:'QA Codex',configDir:'C:/qa',createdAt:1}]}));
 ...
 await evaluate("document.querySelector('.terminal-picker > button').click()");await delay(150);
 results.profile=await evaluate("document.querySelector('.profile-picker')?.innerText.includes('QA Codex')");
 if(!results.profile)throw Error('Profile submenu failed');
 await evaluate("document.querySelector('.profile-picker').dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}))");await delay(100);
 results.profileClosed=await evaluate("!document.querySelector('.profile-picker') && document.activeElement===document.querySelector('.terminal-picker > button')");
 if(!results.profileClosed)throw Error('Profile focus restoration failed');
 ...
 if(results.settingsOrder[0]!=='codex'||!results.after[0].startsWith('Codex')||!results.reloaded[0].startsWith('Codex'))throw Error('Reorder or persistence failed');
