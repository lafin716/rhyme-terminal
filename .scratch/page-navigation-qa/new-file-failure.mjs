...
const server = http.createServer(async (req,res) => {
 if(req.url.includes('/assets/FileViewer-') && req.url.endsWith('.js')){res.writeHead(503).end();return;}
...
try {
 ...
 console.log(JSON.stringify({errors,editor:await evaluate("!!document.querySelector('.monaco-editor, .fallback-editor')")},null,2)); if(!await evaluate("!!document.querySelector('.monaco-editor, .fallback-editor')"))throw Error("New file has no usable editor when the editor module fails to load");
